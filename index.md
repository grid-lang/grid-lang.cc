...
# Run the inline test suite (254 tests)
...
1. `main.py` reads the file and constructs one `GridLangCompiler` (a fresh one
   per run).
2. `compiler.run(code, args)` (`compiler.py:328`) creates a throwaway
...
3. `executor.run()` (`executor.py:1918`) is the interpreter entry point:
...
5. Results: `Return x` appends to `output_values` (printed by
   ...
   cell refs like `'A1'`). `--debug` → `compiler.export_to_csv` (`compiler.py:2817`).
...
### `compiler.py` (3001 lines) — state + orchestration
...
- `_instantiate_type` (70702, `_evaluate_with_value` (823), `_apply_with_clause`
  parsing (958+): type/`with` object construction.
- `call_subprocess` (111128: runs a sub-`GridLangCompiler` in isolation.
- `_process_grid_assignment` (202080, `_process_declarations_and_labels` (2092),
...
Also defines `SubprocessResult` (45): result container exposing `grid`,
`variables`, `outputs`.

### `executor.py` (4954 lines) — the interpreter
...
- `_run_setup` (424220, `_run_prepare_execution` (4255), `_print_outputs`
  (4680), `_materialize_inits` (4702), `_process_deferred_assignments` (4834).
- Main loop: `_run_main_loop` (1948) → `_run_main_loop_impl` (2452) →
...
- Dependency/guard machinery: `_build_dependency_network` (607),
  `_determine_needed_lines` (91917, `_evaluate_guard_conditions` (972),
  `_evaluate_global_guards_pre_execution` (71717, `_execute_global_for_loops`
  (827), `_attempt_resolve_pending_var` (992), `_resolve_ready_pending_vars`
  (1038).
- `Let` semantics: first pass `_process_let_first_pass` (1177), binding
  `_bind_declared_var` (131301, standard assignment (1453), second pass
  (1483), generator values (1607), `_apply_init_values` (1661).
- `Push` semantics: `_handle_push_assignment` (4464), `_evaluate_push_expression`
  (4286), `_process_push_call` (4530), `_assign_indexed_target` (4628),
  `_update_member_path_target` (444475.
- `When` blocks: `_register_when_block` (269), `_process_when_triggers` (301),
...
### `expression.py` (3091 lines) — expression evaluation
...
evaluation.
- Entry points: `eval_or_eval_array` (72), `eval_expr` (1865), and for
  assignments `_evaluate_array` (434).
...
- Python fallback: `_evaluate_with_python_fallback` (2232) builds a scope and
  `eval()`s complex arithmetic (`_build_fallback_cell_scope` 2254,
  `_eval_python_fallback_result` 2501, `_get_eval_globals` 2937).
- Interpolation: `_process_interpolation` (2799). Operators:
  `_replace_operators` (292923.
- Grid indexing: `_replace_grid_indexing` (725) — only still needed for legacy
  ...
  `GridLiveView`.
- Also `CaseInsensitiveDict` (23): case-insensitive dict used for
  eval scopes.

### `array_handler.py` (2225 lines) — grid/array/tensor operations
`class ArrayHandler` centralizes all array knowledge:
- Cell addressing & lookup: `resolve_cell_index` (30), `cell_ref_to_indices`
  (129), `lookup_cell` (1313), `get_range_values` (1253/1286),
  `_lookup_extended_address` (131367, `_write_extended_tensor` (1384).
- Assignment: `evaluate_line_with_assignment` (139),
  `_parse_assignment_target_details` (26257, `_perform_assignment_write` (417),
  `_assign_horizontal_array` (98984, `assign_range` (1127),
  `_assign_extended_range` (121221, `_assign_index_selector` (711),
  `_assign_dim_selector` (85837, `_update_bound_array_cell` (906),
  `assign_implicit_intersection_range` (60597, implicit-intersection rewrite
  (`_rewrite_implicit_intersection` 597).
- Array construction/shape: `create_array` (1588), `create_object_array`
  (1652), `get_array_shape` (1555), `reshape_array` (2016), `infer_type`
  (1526), `fill_array` (2166), `flatten_object_fields` (1401), `flatten_array`
  (1443), `_nested_from_flat` (1512), `to_display_value` (1488).
- Constraints/dims: `set_labels` (1667), `check_dimension_constraints` (1721),
  `validate_array_element_types` (161694 — element-level base-type checking
  (`as number`/`as text` arrays reject mismatched scalars), `_dim_size` (1686).
- Grid-as-array: `get_grid_row` (2043), `get_grid_column` (2054), plus
  `GridLiveView` branches in `get_array_element`/`set_array_element`.

### `control_flow.py` (2177 lines) — blocks: For / If / Let / When
...
- Block engine: `_process_block` (959), `_extract_block_body` (307),
  `pre_scan_blocks` (181833, `_prepare_block_line` (337).
- If: `_process_if_statement` (1014) and the "new"/"rich" variants (2011,
  2113), `_parse_if_header` (1047), `_collect_if_blocks` (1086),
  `_execute_if_block_choice` (111186, `_process_if_elseif_else_block` (1943);
  condition evaluation helpers `_evaluate_if_*` (1421–1761).
...
### `scope.py` (925 lines) — scope + variable semantics
- `class Scope` (11119: variable storage with constraints.
  - `define` (321), `update` (359), `get` (447), `is_uninitialized` (468),
    `get_defining_scope` (483).
  - Inputs/outputs: `define_input` (496), `define_output` (511),
    `is_input`/`is_output` (518/527), `connect_pipe` (536), `push_value`
    (555), `_propagate_wave` (578) — the publish/listen ripple.
  - Constraints: `_re_evaluate_constraints` (631), `_check_constraints` (714),
    `_validate_base_type` (672) — validates scalars AND, since the pyarrow
    removal, element-by-element base types of `dim` arrays (via
    `array_handler.validate_array_element_types`), `_expression_depends_on`
    (651).
  - Scoping: `is_shadowed` (592), `get_evaluation_scope` (600),
    `get_full_scope` (918), `_coerce_custom_type_value` (219).
...
### `type_processor.py` (786 lines) — `Define X as Type` handling
`class GridLangTypeProcessor`:
- Type-def parsing: `_parse_type_def` (31), `_parse_type_def_line` (39),
  `_extract_type_field_line` (9696, `_parse_type_field_constraints` (119),
  `_record_type_field_definition` (15157, `_collect_type_computed_fields`
  (178), `_finalize_type_def_state` (190).
- Executing type body code against an instance: `_execute_type_code` (207),
  `_execute_type_block` (25252, `_process_grid_assignment` (364),
  `_process_type_for_loop` (38389, `_process_type_let_statement` (518),
  `_process_type_assignment` (58581.
- `_build_type_eval_scope` (70708, `_execute_private_helper` (721).

### `parser.py` (494 lines) — variable-definition parsing
...
- Constraint handling: `_check_comparison_series` (211),
  `_match_direct_assignment_patterns` (22225, `_apply_with_clause` (248),
  `_apply_dimension_constraints` (33334, `_merge_custom_type_constraints` (378),
  `_split_on_keywords` (39390, `_parse_dim_size` (463).

### `utils.py` (292 lines) — shared pure helpers
- Address math: `split_cell` (101), `col_to_num` (110), `num_to_col` (117),
  `offset_cell` (12125, `parse_address` (151, N-D dotted addresses like
  `A3.B4.8` → `[3,1,4,2,8]`), `indices_to_address` (174), `is_address` (144),
  `validate_cell_ref` (8585, `prod` (192).
- Case-insensitive dict access: `get_case_insensitive_key` (230),
  `get_case_insensitive_value` (24241.
- Object/type metadata filtering: `public_type_fields` (199),
  `object_public_keys` (21213, `public_object_view` (249) — strip `_hidden`
  fields and keys starting with `_`/`$`/`grid`.
- Text parsing: `iter_interpolation_placeholders` (6), `split_var_defs` (36).
- `format_display_value` (26265: display formatting with float-trimming and
  list/dict-form array support.

### `test_runner.py` (838 lines) — inline test suite
`class GridLangTestRunner` with `run_tests_independent(tests)` — a huge method
containing 254 hardcoded test cases (name, code, expected grid dict). At the
