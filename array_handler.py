...
class ArrayHandler:
    ...
    def resolve_cell_index(self, var_name, cell_ref, line_number=None):
        ...
        # Validate dimensions (expecting [Dept: *, Quarter: 4])
        dims = self.compiler.dimensions.get(var_name, [])
        if len(dims) != 2 or dims[1][1] != 4:
            raise ValueError(
                f"Expected 2D array with Quarter dimension of size 4 for '{var_name}' at line {line_number}")

        # Access the element
        try:
            if quarter_index < 0 or quarter_index >= 4:
                raise IndexError(
                    f"Quarter index {quarter_index} out of bounds for dimension size 4 at line {line_number}")
            if not arr_pylist or not isinstance(arr_pylist[0], list):
                raise ValueError(
                    f"Array '{var_name}' is not properly shaped at line {line_number}")
            # Assuming first dimension size 1
            value = arr_pylist[0][quarter_index]
            return value
    ...
    def evaluate_line_with_assignment(self, line, line_number=None, scope=None):
        ...
        if '[' in target_part and ']' in target_part and not target_part.startswith('[') and '!' not in target_part:
            ...
            if re.match(r'^[A-Z]+\d+$', inside):
                ...
                rm = re.match(r'^([A-Z]+\d+)\s*:\s*([A-Z]+\d+)$', inside)
                if rm:
                    sr, er = rm.groups()
                    try:
                    ...
                    try:
                        ...
                        rm = re.match(
                            r'^([A-Z]+\d+)\s*:\s*([A-Z]+\d+)$', final_inside)
                        if rm:
                            sr, er = rm.groups()
                            validate_cell_ref(sr)
                            validate_cell_ref(er)
                ...
                try:
                    final_inside = self.compiler.expr_evaluator._process_interpolation(
                        f'$"{inside}"', scope, line_number)
                    if re.match(r'^[A-Z]+\d+$', final_inside):
        ...
        # Evaluate RHS
        try:
            if '+' in expr_part and expr_part.count('{') >= 2:
        ...
        if is_harr and expr_part.startswith('{') and expr_part.endswith('}'):
            ...
            if all(elem in self.compiler.variables for elem in elements):
                first_elem = self.compiler.variables[elements[0]]
                ...
                if object_type_name:
                    is_array_of_two_field_objects = all(
                        isinstance(self.compiler.variables[elem], dict) and
                        set(self.compiler.variables[elem].keys()) == set(
    ...
    def _assign_index_selector(self, var_name, indices, value, line_number=None):
        ...
        if len(indices) == 1 and re.match(r'^[A-Z]+\d+$', indices[0]):
            cell_ref = indices[0]
            ...
            if value is None:  # Read
                ...
                self.compiler.variables[var_name] = pa.ListArray.from_arrays(
                    offsets=pa.array(
            ...
            if len(shape) > 1:
                ...
                self.compiler.variables[var_name] = new_values
    ...
    def set_array_element(self, array, indices, value, line_number=None):
        """
        Set an element in a pyarrow ListArray of any dimension.
        :param array: Pyarrow ListArray.
        ...
        if not isinstance(array, pa.ListArray):
            raise TypeError(
                f"Expected ListArray, got {type(array)} at line {line_number}")
        ...
        # Calculate flat index for any dimension
        flat_idx = 0
        stride = 1
        ...
        flat_arr[flat_idx] = value
        pa_type = array.type.value_type if isinstance(
            array.type, pa.ListType) else pa.float64()
        new_values = pa.array(flat_arr, type=pa_type)

        # Reconstruct nested ListArray for arbitrary dimensions
        def create_nested_array(values, shape, depth):
            if depth == len(shape) - 1:
                return pa.ListArray.from_arrays(
                    pa.array([i * shape[depth]
                              for i in range(shape[depth - 1] + 1)], type=pa.int32()),
                    values
                )
            inner_array = create_nested_array(values, shape, depth + 1)
            offsets = [i * shape[depth] for i in range(shape[depth - 1] + 1)]
            return pa.ListArray.from_arrays(pa.array(offsets, type=pa.int32()), inner_array)

        # Handle 1D case separately
        if len(shape) == 1:
            return pa.array(flat_arr, type=pa_type)
        # Handle multi-dimensional case
        return create_nested_array(new_values, shape + (1,), 1)
    ...
    def get_array_element(self, arr, indices, line_number=None, return_struct=False, original_shape=None, is_grid_dim=False):
        """
        Get an element from an array using indices.
        Supports pyarrow and list arrays, with optional grid dimension handling.
        :param arr: Array or dict with 'array' key.
        :param indices: List of indices (1-based for grid_dim).
        :param line_number: Line number.
        :param is_grid_dim: Use grid dimension logic if True.
        :param return_struct: Return struct if True (for grid_dim).
        :param original_shape: Original shape if provided (for grid_dim).
        :return: Element value.
        """
        if is_grid_dim and isinstance(arr, dict) and 'array' in arr and isinstance(arr['array'], pa.ListArray):
            arr = arr['array']
            original_shape = arr['original_shape'] if original_shape is None else original_shape
        shape = self.get_array_shape(arr, line_number)
        original_shape = original_shape if original_shape is not None and is_grid_dim else getattr(
            arr, 'original_shape', shape)

        # Adjust indices for grid_dim (1-based to 0-based)
        if is_grid_dim:
        ...
        # Validate indices
        if (is_grid_dim and len(indices) > len(original_shape)) or (not is_grid_dim and len(indices) != len(shape)):
            expected = len(original_shape) if is_grid_dim else len(shape)
            raise ValueError(
                f"Expected {'up to ' if is_grid_dim else ''}{expected} indices for array with shape {original_shape if is_grid_dim else shape}, got {len(indices)} at line {line_number}")

        for i, idx in enumerate(indices):
            dim_size = original_shape[i] if is_grid_dim else shape[i]
            if idx < 0 or idx >= dim_size:
                raise IndexError(
                    f"Index {idx} out of bounds for dimension {i} with size {dim_size} at line {line_number}")

        # Handle array indexing
        if isinstance(arr, pa.ListArray):
            if is_grid_dim and len(original_shape) == 3 and len(indices) == 3:
                i, j, k = indices
                rows, cols, depth = original_shape
                flat_idx = k * (rows * cols) + i * cols + j
                flat_array = arr.flatten()
                if flat_idx < 0 or flat_idx >= len(flat_array):
                    raise IndexError(
                        f"Calculated index {flat_idx} out of bounds for array length {len(flat_array)} at line {line_number}")
                result = flat_array[flat_idx].as_py()
                return result
            else:
                flat_arr = arr.flatten().to_pylist()
                flat_idx = indices[0] * shape[1] * \
                    shape[2] + indices[1] * shape[2] + indices[2]
